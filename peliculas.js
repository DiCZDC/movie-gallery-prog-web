...
// Función para buscar imagen en TMDb
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      return posterPath 
        ...
        : `https://via.placeholder.com/220x300/333/fff?text=${encodeURIComponent(titulo)}`;
    }
    return `https://via.placeholder.com/220x300/333/fff?text=${encodeURIComponent(titulo)}`;
  } catch (error) {
...
function crearCard(container,text, content){
    let title = document.createElement("h1");
    title.textContent = text;
    let title_var = document.createElement("p");
    title_var.textContent = content;
    container.appendChild(title);
    container.appendChild(title_var); 
}

peliculas.forEach(async pelicula => {
    //Img
    const img = document.createElement("img");
    img.src = `https://via.placeholder.com/220x300/666/fff?text=Cargando...`;
    img.alt = pelicula.titulo;
    const imagenUrl = await buscarImagenPelicula(pelicula.titulo, pelicula.año);
    img.src = imagenUrl;
    //tarjeta
